...
A backend for application that allows users to **fetch, explore, and save job offers**.  
Access to these features requires authentication via **JWT tokens**.  
The application runs in a **Docker** environment with **MongoDB** as the database and **Redis** for caching/session management.
...
|------------|-------------|
| **Backend** | Spring Boot Web, Data MongoDB, Validation |
| **Scheduler** | Spring Scheduler |
| **Database** | MongoDB |
| **Cache** | Redis |
| **Security** | Spring Boot Security + JWT |
| **Containerization (Redis & MongoDB)** | Docker & Docker Compose |
| **API Format** | REST (JSON) |
| **Unit Tests** | Junit5, AssertJ |
| **Integration Tests** | Spring Boot Test, Testcontainers, WireMock, Awaitility |
...
16. user made POST /offers with header “Authorization: Bearer AAAA.BBBB.CCC” and offer and system returned CREATED(201) with saved offer
...
This is the list of modules with their responsibilities:

Offers
- crud for job offers
- periodically fetching offers from external service
...
![Joboffers Components](architecture/joboffers_components.drawio.png)
...
```
git clone https://github.com/faust2077/joboffers.git
cd joboffers
```
...
This command starts the MongoDB and Redis containers.

### Run the application

 1. Directly from IDE by navigating to src/main/java/com/joboffers/JobOffersApplication class and running the main method.
